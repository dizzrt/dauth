...
const OperationUserServiceGetUserByName = "/UserService/GetUserByName"
...
type UserServiceHTTPServer interface {
	...
	// GetUserByName GetUserByName gets a user by username.
...
func RegisterUserServiceHTTPServer(hs *http.Server, srv UserServiceHTTPServer) {
	...
	r.GET("/identity/user/name/:username", _user_UserService_GET_GetUserByName_HTTP_Handler(hs, srv))
...
func _user_UserService_GET_GetUserByName_HTTP_Handler(hs *http.Server, srv UserServiceHTTPServer) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		tracer := otel.Tracer(TRACER_NAME_USER)
		rctx, span := tracer.Start(rctx, "_UserService_GetUserByName_0_HTTP_Handler",
			trace.WithSpanKind(trace.SpanKindServer),
			trace.WithAttributes(attributes...),
		)
		defer span.End()
		...
		res, err := srv.GetUserByName(rctx, &req)
		ctx.Request = ctx.Request.WithContext(rctx)
		if err != nil {
			ctx.JSON(http.HTTPStatusCodeFromError(err), hs.WrapHTTPResponse(res, err))
			ctx.Abort()
